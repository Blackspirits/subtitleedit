...
public class OmniVoiceDownloadServiceTests
{
    ...
    public async Task DownloadEngine_TamperedPayload_IsRejectedAndRewound()
    {
        ...
        {
            Assert.Skip("OmniVoice runtime is not supported on this operating system.");
        }

        using var httpClient = new HttpClient(new StaticResponseHandler(Encoding.ASCII.GetBytes("tampered")));
        var service = new OmniVoiceDownloadService(httpClient);
    ...
    public async Task VerifyArchive_EmptyStream_IsRejectedAndRewound()
    {
        ...
        await Assert.ThrowsAsync<IOException>(() =>
            OmniVoiceDownloadService.VerifyArchive(
                stream,
                DownloadHashManager.OmniVoice.LinuxX64,
                "engine",
                TestContext.Current.CancellationToken));
