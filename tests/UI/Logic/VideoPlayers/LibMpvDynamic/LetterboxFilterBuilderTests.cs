...
public class LetterboxFilterBuilderTests
{
    ...
    [Fact]
    public void LabelledEntries_HaveStableIndependentLabels()
    {
        var settings = new SeVideoLetterbox { Enabled = true, TopHeightPercent = 10, BottomHeightPercent = 12, Color = "#000000" };

        var (top, bottom) = LetterboxFilterBuilder.BuildLabelledEntries(settings);

        Assert.NotNull(top);
        Assert.NotNull(bottom);
        Assert.StartsWith("@se-letterbox-top:lavfi=[drawbox=", top);
        Assert.StartsWith("@se-letterbox-bottom:lavfi=[drawbox=", bottom);
    }
