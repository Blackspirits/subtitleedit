...
internal static class PaddleOcrDownloadIntegrity
{
    ...
    {
        return Hashes.TryGetValue(Path.GetFileName(url), out var hash) ? hash : null;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // Best effort: the failed download must never be unpacked even if cleanup fails.
        }

        throw new IOException(
            $"Paddle OCR download failed integrity check (expected SHA-256 {expected}, got {actual}).");
    }
}
