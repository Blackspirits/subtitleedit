...
{
    private static readonly IReadOnlyDictionary<string, string> Sha256ByFileName =
    ...
    internal static string? GetExpectedSha256(string url)
    {
        ...
        return Sha256ByFileName.TryGetValue(fileName, out var hash) ? hash : null;
    }
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(actual) || !string.Equals(expected, actual, StringComparison.OrdinalIgnoreCase))
            {
                throw new IOException(
                    ...
                    $"(expected SHA-256 {expected}, got {actual ?? "<missing>"}).");
            }
        }
        catch
        {
    ...
    private static void TryDelete(string fileName)
    {
        ...
        {
            // Best effort: never hide the download/integrity failure behind cleanup.
        }
    }
}
