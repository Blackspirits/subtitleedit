...
public abstract class CrispAsrEngineBase : ICrispAsrEngine
{
    ...
    /// users off the CPU one. Figures are the v0.8.33 release assets (the two Linux CUDA
    /// tarballs still come from v0.8.32, which is where they were last published) —
    ...
    public virtual string DownloadSizeText
    {
        get
        {
            ...
            {
                // CPU ~8 MB (legacy ~8 MB), Vulkan ~35 MB, CUDA 13 ~486 MB, CUDA 12 ~692 MB.
                return "~8 MB – 692 MB";
            }
            if (OperatingSystem.IsLinux())
            {
                // CPU ~38 MB (arm64 ~32 MB), Vulkan ~70 MB, ROCm ~102 MB, CUDA 13 ~120 MB, CUDA 12 ~155 MB.
                return "~32 MB – 155 MB";
            }
