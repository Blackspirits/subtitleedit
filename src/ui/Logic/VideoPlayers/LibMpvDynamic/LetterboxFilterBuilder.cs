...
{
    /// <returns>
    /// A comma-joined drawbox representation, or an empty string when letterboxing is off.
    /// Runtime mpv integration uses <see cref="BuildLabelledEntries"/> so it can manage only
    /// Subtitle Edit's own filters without replacing the caller's complete <c>vf</c> chain.
    /// </returns>
    ...
    public static (string? Top, string? Bottom) BuildLabelledEntries(SeVideoLetterbox settings)
    {
        if (!settings.Enabled)
        {
            return (null, null);
        ...
        var top = topFraction > 0
            ? $"@se-letterbox-top:lavfi=[drawbox=x=0:y=0:w=iw:h=ih*{topFraction.ToString(CultureInfo.InvariantCulture)}:color={color}:t=fill]"
            : null;
        var bottom = bottomFraction > 0
            ? $"@se-letterbox-bottom:lavfi=[drawbox=x=0:y=ih-ih*{bottomFraction.ToString(CultureInfo.InvariantCulture)}:w=iw:h=ih*{bottomFraction.ToString(CultureInfo.InvariantCulture)}:color={color}:t=fill]"
            : null;
