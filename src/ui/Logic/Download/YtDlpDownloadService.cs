...
public interface IYtDlpDownloadService
{
    ...
    /// <summary>
    /// Downloads the auto-generated subtitle in the video's original language
    /// (yt-dlp's <c>{lang}-orig</c> track) to <paramref name="outputStem"/>.<c>{lang}-orig.{ext}</c>.
    /// Only the original-language track is fetched - requesting "all" auto subtitles
    /// would download every machine-translated language (150+ files on YouTube).
    /// </summary>
    Task DownloadAutoGeneratedSubtitlesAsync(string url, string outputStem, CancellationToken cancellationToken);
...
public class YtDlpDownloadService : IYtDlpDownloadService
{
    ...
    private async Task DownloadSubtitlesOnlyAsync(string url, string outputStem, bool autoGenerated, CancellationToken cancellationToken)
    {
        ...
        var args = new List<string>
        {
            ...
            "--no-warnings",
            "--no-playlist",
            // Auto captions: only the original-language track (yt-dlp names it
            // "{lang}-orig") - "all" would fetch every machine translation.
            autoGenerated ? "--write-auto-subs" : "--write-subs",
            "--sub-langs", autoGenerated ? ".*-orig" : "all",
            "--sub-format", "vtt/best",
            "-o", outputStem,
            url,
        };
