...
public class KokoroTtsCppDownloadService : IKokoroTtsCppDownloadService
{
    ...
    internal static async Task VerifyArchive(Stream stream, string? key, string label, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException($"No SHA-256 key is registered for Kokoro TTS {label}.");
        }

        var expected = DownloadHashManager.GetLatestKnownHash(key);
        if (string.IsNullOrEmpty(expected))
        {
            throw new InvalidOperationException($"No SHA-256 is registered for Kokoro TTS {label} key '{key}'.");
        }

        if (!stream.CanRead || !stream.CanSeek)
        {
            throw new InvalidOperationException($"Kokoro TTS {label} integrity verification requires a readable, seekable stream.");
        }
