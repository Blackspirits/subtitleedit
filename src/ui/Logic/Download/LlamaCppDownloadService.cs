...
public class LlamaCppDownloadService(HttpClient httpClient) : ILlamaCppDownloadService
{
    ...
    internal static async Task VerifyArchive(Stream stream, string? key, string label, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException($"No SHA-256 key is registered for llama.cpp {label}.");
        }

        var expected = DownloadHashManager.GetLatestKnownHash(key);
        if (string.IsNullOrEmpty(expected))
        {
            throw new InvalidOperationException($"No SHA-256 is registered for llama.cpp {label} key '{key}'.");
        }

        if (!stream.CanRead || !stream.CanSeek)
        {
            throw new InvalidOperationException($"llama.cpp {label} integrity verification requires a readable, seekable stream.");
        }
