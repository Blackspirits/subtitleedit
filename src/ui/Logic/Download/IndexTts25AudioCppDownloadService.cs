...
public class IndexTts25AudioCppDownloadService : IIndexTts25AudioCppDownloadService
{
    ...
    internal static async Task VerifyFile(string filePath, string? key, string fileName, CancellationToken cancellationToken)
    {
        if (string.IsNullOrEmpty(key))
        {
            throw new InvalidOperationException($"No SHA-256 key is registered for IndexTTS 2.5 model '{fileName}'.");
        }
        ...
        if (string.IsNullOrEmpty(expected))
        {
            throw new InvalidOperationException(
                $"No SHA-256 is registered for IndexTTS 2.5 model '{fileName}' key '{key}'.");
        }
